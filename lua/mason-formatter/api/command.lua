...
local function parse_packages_from_user_args(user_args)
	...
	return _.filter_map(function(source_specifier)
		...
		-- 1. first see if the provided arg is an actual formatter source name
		return Optional
			.of_nilable(source_mappings.formatter_to_package[source_name])
			...
			:or_(function()
				return Optional.of_nilable(language_mappings[source_name]):map(function(package_names)
					local package_names = _.filter(function(package_name)
						return source_mappings.formatter_to_package[package_name] ~= nil
					end, package_names)
					...
					return a.promisify(vim.ui.select)(package_names, {
						...
						format_item = function(package_name)
							local source_name = source_mappings.formatter_to_package[package_name]
			...
			:if_not_present(function()
				notify(("Could not find Formatter Source %q."):format(source_name), vim.log.levels.ERROR)
			end)
	end, user_args)
end
...
local function parse_packages_from_heuristics()
	...
	local filetype_mappings = require("mason-formatter.settings").current.filetype_mappings
	return Optional.of_nilable(filetype_mappings[current_ft])
		:map(function(source_names)
			return a.promisify(vim.ui.select)(source_names, {
				...
				format_item = function(source_name)
					if registry.is_installed(source_mappings.formatter_to_package[source_name]) then
						return ("%s (installed)"):format(source_name)
					else
						return source_name
					end
		...
		:map(function(source_name)
			local package_name = source_mappings.formatter_to_package[source_name]
			return { { package = package_name, version = nil } }
		end)
		:or_else_get(function()
			notify(("No Formatter sources found for filetype %q."):format(current_ft), vim.log.levels.ERROR)
			return {}
		end)
end
...
local FormatterInstall = a.scope(function(sources)
...
vim.api.nvim_create_user_command("FormatterInstall", function(opts)
	...
	desc = "Install one or more Formatter sources.",
	nargs = "*",
	complete = "custom,v:lua.mason_formatter_completion.available_source_completion",
})

local function FormatterUninstall(sources)
	require("mason.ui").open()
	require("mason.ui").set_view("All")
	local registry = require("mason-registry")
	local source_mappings = require("mason-formatter.mappings.sources")
	for _, source_specifier in ipairs(sources) do
		local package_name = source_mappings.formatter_to_package[source_specifier]
...
vim.api.nvim_create_user_command("FormatterUninstall", function(opts)
	...
	desc = "Uninstall one or more Formatter sources.",
	nargs = "+",
	complete = "custom,v:lua.mason_formatter_completion.installed_source_completion",
})

_G.mason_formatter_completion = {
	available_source_completion = function()
		local available_sources = require("mason-formatter").get_available_sources()
		local language_mappings = require("mason.mappings.language")
	...
	installed_source_completion = function()
		local installed_sources = require("mason-formatter").get_installed_sources()
