...
---@param config MasonFormatterSettings | nil
M.setup = function(config)
	...
	if #settings.current.ensure_installed > 0 then
		require("mason-formatter.ensure_installed")()
	end
...
function M.setup_handlers(handlers)
	...
	_.each(function(handler)
		if type(handler) == "string" and not source_mappings.formatter_to_package[handler] then
			notify(
				("mason-formatter.setup_handlers: Received handler for unknown formatter source name: %s."):format(
					handler
				),
				vim.log.levels.WARN
			)
	...
	local function get_source_name(pkg_name)
		return Optional.of_nilable(source_mappings.package_to_formatter[pkg_name])
	end
...
function M.get_available_sources(filter)
	...
	return _.filter_map(function(pkg_name)
		return Optional.of_nilable(source_mappings.package_to_formatter[pkg_name]):map(function(source_name)
